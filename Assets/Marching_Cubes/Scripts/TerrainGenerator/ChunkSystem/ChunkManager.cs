...
public class ChunkManager : MonoBehaviour
{
    ...
    [Tooltip(
        "Use the camera position to calculate the player position. True-> use Camera.main tag / False-> use Player tag")]
    public bool useCameraPosition = true;

    [Tooltip(
        "F4 to active. Show the current chunk and the data of the voxel you are looking. Important: You need activate gizmos in Game tab!!")]
    ...
    {
        Debug.Log("Initializing chunkmanager");
        ...
        lastPlayerPos.x = Mathf.FloorToInt(player.position.x / loadRegionDistance) * loadRegionDistance +
                          loadRegionDistance / 2;
        lastPlayerPos.z = Mathf.FloorToInt(player.position.z / loadRegionDistance) * loadRegionDistance +
                          loadRegionDistance / 2;
        initRegion(Mathf.FloorToInt(player.position.x / loadRegionDistance),
            Mathf.FloorToInt(player.position.z / loadRegionDistance));
    }
    ...
    void HiddeRemoveChunk()
    {
        ...
        {
            float distance = Mathf.Sqrt(Mathf.Pow((player.position.x - chunk.Value.transform.position.x), 2) +
                                        Mathf.Pow((player.position.z - chunk.Value.transform.position.z), 2));
    ...
    void CheckNewChunks()
    {
        Vector2Int actualChunk = new Vector2Int(
            Mathf.CeilToInt((player.position.x - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE),
        ...
        for (int x = actualChunk.x - chunkViewDistance; x < actualChunk.x + chunkViewDistance; x++)
        {
            ...
            {
                if (Mathf.Pow((actualChunk.x - x), 2) + Mathf.Pow((actualChunk.y - z), 2) >
    ...
    void LoadChunkFromList()
    {
        ...
        Vector2Int regionPos = new Vector2Int(Mathf.FloorToInt(((float)key.x) / Constants.REGION_SIZE),
            Mathf.FloorToInt(((float)key.y) / Constants.REGION_SIZE));
        if (
            !regionDict.ContainsKey(
                regionPos)) //In case that the chunk isn't in the loaded regions we remove it, tp or too fast movement.
        {
        ...
        Vector2Int keyInsideChunk = new Vector2Int(key.x - regionPos.x * Constants.REGION_SIZE,
            key.y - regionPos.y * Constants.REGION_SIZE);
        ...
        if (chunkIndexInRegion != 0) //Load chunk from a region data
            chunkDict.Add(key,
                chunkObj.AddComponent<Chunk>().ChunkInit(regionDict[regionPos].GetChunkData(chunkIndexInRegion),
                    keyInsideChunk.x, keyInsideChunk.y, regionDict[regionPos], false));
        else //Generate chunk with the noise generator
            chunkDict.Add(key,
                chunkObj.AddComponent<Chunk>().ChunkInit(noiseManager.GenerateChunkData(key), keyInsideChunk.x,
                    keyInsideChunk.y, regionDict[regionPos], Constants.SAVE_GENERATED_CHUNKS));
    ...
    void CheckRegion()
    {
        if (Mathf.Abs(lastPlayerPos.x - player.position.x) > loadRegionDistance ||
            Mathf.Abs(lastPlayerPos.z - player.position.z) > loadRegionDistance)
        {
    ...
    public void ModifyChunkData(Vector3 modificationPoint, float range, float modification, int mat = -1)
    {
        ...
        modificationPoint = new Vector3(modificationPoint.x / Constants.VOXEL_SIDE,
            modificationPoint.y / Constants.VOXEL_SIDE, modificationPoint.z / Constants.VOXEL_SIDE);

        //Chunk voxel position (based on the chunk system)
        Vector3 vertexOrigin =
            new Vector3((int)modificationPoint.x, (int)modificationPoint.y, (int)modificationPoint.z);

        //intRange (convert Vector3 real world range to the voxel size range)
        int intRange =
            (int)(range / 2 * Constants.VOXEL_SIDE); //range /2 because the for is from -intRange to +intRange
        ...
        for (int y = -intRange; y <= intRange; y++)
        {
            for (int z = -intRange; z <= intRange; z++)
            {
                for (int x = -intRange; x <= intRange; x++)
                {
                    ...
                    if (vertexOrigin.y + y >= Constants.MAX_HEIGHT / 2 ||
                        vertexOrigin.y + y <= -Constants.MAX_HEIGHT / 2)
                        continue;
                    ...
                    Vector2Int hitChunk = new Vector2Int(
                        Mathf.CeilToInt((vertexPoint.x + 1 - Constants.CHUNK_SIZE / 2) / Constants.CHUNK_SIZE),
                    ...
                    Vector3Int vertexChunk = new Vector3Int(
                        (int)(vertexPoint.x - hitChunk.x * Constants.CHUNK_SIZE + Constants.CHUNK_VERTEX_SIZE / 2),
                    ...
                    if (vertexChunk.x == 0 &&
                        vertexChunk.z == 0) //Interact with chunk(-1,-1), chunk(-1,0) and chunk(0,-1)
                    {
                        //Vertex of chunk (-1,0)
    ...
    {
        point = new Vector3(point.x / Constants.VOXEL_SIDE, point.y / Constants.VOXEL_SIDE,
            point.z / Constants.VOXEL_SIDE);

        Vector3 vertexOrigin = new Vector3((int)point.x, (int)point.y, (int)point.z);

        //Chunk containing the point
        Vector2Int hitChunk = new Vector2Int(
            Mathf.CeilToInt((vertexOrigin.x + 1 - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE),
            Mathf.CeilToInt((vertexOrigin.z + 1 - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE));
        //VertexPoint of the point in the chunk (x,y,z)
        Vector3Int vertexChunk = new Vector3Int(
            (int)(vertexOrigin.x - hitChunk.x * Constants.CHUNK_SIZE + Constants.CHUNK_VERTEX_SIZE / 2),
        ...
        else //Loop next vertex for get a other material different to air
        {
            ...
            {
                //Chunk of the vertexPoint
                hitChunk = new Vector2Int(
                    Mathf.CeilToInt((nextVertexPoints[i].x + 1 - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE),
                    Mathf.CeilToInt((nextVertexPoints[i].z + 1 - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE));
                //Position of the vertexPoint in the chunk (x,y,z)
                vertexChunk = new Vector3Int(
                    (int)(nextVertexPoints[i].x - hitChunk.x * Constants.CHUNK_SIZE + Constants.CHUNK_VERTEX_SIZE / 2),
                ...
                if (chunkDict[hitChunk].GetMaterial(vertexChunk) !=
                    Constants.NUMBER_MATERIALS) //not air material, we return it
                {
                    return chunkDict[hitChunk].GetMaterial(vertexChunk);
                }
            }
    ...
    private void OnDrawGizmos()
    {
        if (debugMode && Application.isPlaying)
        {
            ...
            Vector2Int actualChunk = new Vector2Int(
                Mathf.CeilToInt((player.position.x - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE),
                Mathf.CeilToInt((player.position.z - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE));
            Vector3 chunkCenter = new Vector3(actualChunk.x * Constants.CHUNK_SIDE, 0,
                actualChunk.y * Constants.CHUNK_SIDE);
            Gizmos.color = Color.yellow;
            Gizmos.DrawWireCube(chunkCenter,
                new Vector3(Constants.CHUNK_SIDE, Constants.MAX_HEIGHT * Constants.VOXEL_SIDE, Constants.CHUNK_SIDE));

            //Show voxel
            RaycastHit hitInfo;
            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hitInfo, 100.0f))
            {
                Vector2Int chunkHit = new Vector2Int(
                    Mathf.CeilToInt((hitInfo.point.x - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE),
                    Mathf.CeilToInt((hitInfo.point.z - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE));
                Vector3Int vertexChunk = new Vector3Int(
                    (int)(hitInfo.point.x - chunkHit.x * Constants.CHUNK_SIZE + Constants.CHUNK_VERTEX_SIZE / 2),
                ...
                Vector3 voxelRealPosition = new Vector3(
                    (Mathf.FloorToInt(hitInfo.point.x / Constants.VOXEL_SIDE)) * Constants.VOXEL_SIDE +
                    Constants.VOXEL_SIDE / 2,
                    (Mathf.FloorToInt(hitInfo.point.y / Constants.VOXEL_SIDE)) * Constants.VOXEL_SIDE +
                    Constants.VOXEL_SIDE / 2,
                    (Mathf.FloorToInt(hitInfo.point.z / Constants.VOXEL_SIDE)) * Constants.VOXEL_SIDE +
                    Constants.VOXEL_SIDE / 2);
